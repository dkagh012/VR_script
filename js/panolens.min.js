  ...
  y.prototype = Object.assign(Object.create(d.Sprite.prototype), {
    ...
    onHoverStart: function (a) {
      if (this.getContainer()) {
        // 컨테이너가 존재하는지 확인
        var b =
          this.cursorStyle || (this.mode === q.NORMAL ? "pointer" : "default"); // 커서 스타일 설정
        var c = this.scaleDownAnimation; // 축소 애니메이션
        var e = this.scaleUpAnimation; // 확대 애니메이션
        var g = this.element; // 현재 요소

        this.isHovering = !0; // 호버 상태 플래그 설정
        this.container.style.cursor = b; // 컨테이너의 커서 스타일 설정

        if (this.animated) {
          c.stop(); // 축소 애니메이션 중지
          e.start(); // 확대 애니메이션 시작
        }
        if (g && a.mouseEvent.clientX >= 0 && a.mouseEvent.clientY >= 0) {
          // 요소가 존재하고, 마우스 이벤트 좌표가 양수인지 확인
          var a = g.left; // 왼쪽 요소
          var b = g.right; // 오른쪽 요소
          var c = g.style;
          if (this.mode === q.CARDBOARD || this.mode === q.STEREO) {
            // 모드가 CARDBOARD 또는 STEREO인 경우
            c.display = "none"; // 현재 요소 숨김
            a.style.display = "block"; // 왼쪽 요소 표시
            b.style.display = "block"; // 오른쪽 요소 표시
            g._width = a.clientWidth; // 왼쪽 요소 너비
            g._height = a.clientHeight; // 왼쪽 요소 높이
          } else {
            // 모드가 CARDBOARD 또는 STEREO가 아닌 경우
            c.display = "block"; // 현재 요소 표시
            a && (a.style.display = "none"); // 왼쪽 요소 숨김
            b && (b.style.display = "none"); // 오른쪽 요소 숨김
            g._width = g.clientWidth; // 현재 요소 너비
            g._height = g.clientHeight; // 현재 요소 높이
          }
        }
      }
    },
    ...
    onHoverEnd: function () {
      if (this.getContainer()) {
        var a = this.scaleDownAnimation; // 축소 애니메이션
        var b = this.scaleUpAnimation; // 확대 애니메이션
        var c = this.element; // 현재 요소

        this.isHovering = !1; // 호버 상태 플래그 해제
        this.container.style.cursor = "default"; // 컨테이너의 커서 스타일을 기본값으로 설정
        ...
        if (c && !this.element.locked) {
          // 요소가 존재하고 요소가 잠겨있지 않은 경우
          var a = c.left; // 왼쪽 요소
          var b = c.right; // 오른쪽 요소
          c.style.display = "none"; // 현재 요소 숨김
          a && (a.style.display = "none"); // 왼쪽 요소 숨김
          b && (b.style.display = "none"); // 오른쪽 요소 숨김
          this.unlockHoverElement(); // 호버 요소 잠금 해제
        }
      }
    },
